...
// inline constexpr float kMinMeanMagnitude = 10.0f; // Larger -> keep only strong edges. Smaller -> keep weak/noisy
// edges. inline constexpr float kMinSegmentLengthPx = 12.0f; // Larger -> discard short segments. Smaller -> keep short
// fragments.
...
static bool can_merge_olson(const DisjointSetUnion &dsu, const CompStats &s, std::size_t ra, std::size_t rb, float KD,
                            float KM) {
    if (ra == rb)
        return false;
...
static void merge_stats_after_union(const DisjointSetUnion &dsu, CompStats &s, std::size_t root_kept,
                                    std::size_t root_absorbed) {
    if (root_kept == root_absorbed)
        return;
...
static void dump_unionfind_snapshot(const fs::path &out_root, std::size_t unions_done, DisjointSetUnion &dsu,
    ...
    libimages::debug_io::dump_image(
        (dir / ("00_components_randomly_colored" + std::string(cfg::kDefaultDumpExt))).string(),
        make_image_component_colorized(dsu, w, h), cfg::kDumpVerbose, cfg::kDumpForceOverwrite);

    // 2) colorized large components
    libimages::debug_io::dump_image(
        (dir / ("01_components_randomly_colored_only_large" + std::string(cfg::kDefaultDumpExt))).string(),
        make_image_component_colorized(dsu, w, h, cfg::kMinComponentPixels), cfg::kDumpVerbose,
...
static std::vector<std::size_t> make_snapshot_schedule(std::size_t max_unions_possible) {
    ...
    if (max_unions_possible >= 10)
        t.push_back(10);

    for (std::size_t x = 100; x <= 1000 && x <= max_unions_possible; x += 100)
        t.push_back(x);

    for (std::size_t x = 10000; x <= max_unions_possible; x *= 10)
        t.push_back(x);
...
int main(int argc, char **argv) {
    try {
        ...
        libimages::debug_io::dump_image(stage_path(out_dir, 2, "gray"), gray, cfg::kDumpVerbose,
                                        cfg::kDumpForceOverwrite);

        // 03: blur
        const image32f blurred = gaussian_blur_gray(gray, cfg::kGaussianSigma);
        libimages::debug_io::dump_image(stage_path(out_dir, 3, "gray_blurred"), blurred, cfg::kDumpVerbose,
                                        cfg::kDumpForceOverwrite);

        // 04..: gradients
        const Gradients gr = sobel_gradients(blurred);
        libimages::debug_io::dump_image(stage_path(out_dir, 4, "sobel_dx_signed"), visualize_signed_to_u8(gr.dx),
                                        cfg::kDumpVerbose, cfg::kDumpForceOverwrite);
        libimages::debug_io::dump_image(stage_path(out_dir, 5, "sobel_dy_signed"), visualize_signed_to_u8(gr.dy),
                                        cfg::kDumpVerbose, cfg::kDumpForceOverwrite);
        libimages::debug_io::dump_image(stage_path(out_dir, 6, "sobel_mag"), gr.mag, cfg::kDumpVerbose,
                                        cfg::kDumpForceOverwrite);
        ...
        edges.reserve(static_cast<std::size_t>((w - 1) * h) + static_cast<std::size_t>(w * (h - 1)) +
        ...
        for (const Edge &e : edges) {
            ...
            if (!can_merge_olson(dsu, stats, ra, rb, cfg::kClusterKD, cfg::kClusterKM))
                continue;
